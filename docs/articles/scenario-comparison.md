...
The most common use case is comparing agricultural nutrient balances
...
Key metrics include: - `n_sources`: Number of nutrient source areas -
`n_sinks`: Number of nutrient sink areas - `n_balanced` /
`n_within_watershed`: Balanced areas - `n_excluded`: Excluded areas
(below cropland threshold) - `total_surplus_kg`: Total nutrient
surplus - `total_deficit_kg`: Total nutrient deficit
...
The summary shows: - Base scenario name - Delta (change) in key
metrics - Percent change from base scenario
...
**Classification Changes:** - `delta_sources`: Change in nutrient source
areas - `delta_sinks`: Change in nutrient sink areas

**Magnitude Changes:** - `delta_surplus`: Change in total surplus (kg) -
`delta_deficit`: Change in total deficit (kg)

**Negative values** mean the second scenario has fewer/less than the
base scenario.
...
Compare scenarios that differ in **one key aspect** for clearest
interpretation:
...
If comparing scenarios with different scales, metrics won’t be directly
comparable:
